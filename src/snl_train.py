...
args.dataset = dataset_str + '/100eta'+str(int(100*args.eta))+args.outlier_style
args.num_trials = 2
args.seeds = [2,9,11,20,40]
whole_map = np.load('../real_data/'+dataset_str+'_data.npy')
...
def evaluation(random_seed, logstr, score, A_torch, Ind_i, Ind_j, Ind_k, label_np, val_index, test_index, SavePred, save_path, split, identifier_str):
    ...
    if identifier_str == '_best':
        ...
        np.save('../'+dataset_str+'_pred/GNNSync_k50_thres6_100eta'+str(int(100*args.eta))+args.outlier_style+'seed'+str(random_seed)+'split'+str(split), score)
        ...
        plt.savefig('../'+dataset_str+'_plots/GNNSync_k50_thres6_100eta'+str(int(100*args.eta))+args.outlier_style+'seed'+str(random_seed)+'split'+str(split)+'.pdf',format='pdf')
        plt.savefig('../'+dataset_str+'_plots/GNNSync_k50_thres6_100eta'+str(int(100*args.eta))+args.outlier_style+'seed'+str(random_seed)+'split'+str(split)+'.png',format='png')
        plt.show()
...
class Trainer(object):
    ...
    def non_nn(self, model_name):
        ...
        for split in range(self.runs):
            ...
            angles_diff = (score-label_np) % (2*np.pi)
            score = (score - angles_diff.mean()) % (2*np.pi)
            np.save('../'+dataset_str+'_pred/'+model_name+'_k50_thres6_100eta'+str(int(100*args.eta))+args.outlier_style+'seed'+str(random_seed)+'split'+str(split), score)
            count_vec = np.zeros(num_nodes)
            x_sum_vec = np.zeros(num_nodes)
            y_sum_vec = np.zeros(num_nodes)
            for patch_idx in range(num_nodes):
            ...
            plt.scatter(x_vec, y_vec, s=2, c='blue')
            plt.scatter(whole_map[:, 0], whole_map[:, 1], s=2, c='red')
            plt.savefig('../'+dataset_str+'_plots/'+model_name+'_k50_thres6_100eta'+str(int(100*args.eta))+args.outlier_style+'seed'+str(random_seed)+'split'+str(split)+'.pdf',format='pdf')
            plt.savefig('../'+dataset_str+'_plots/'+model_name+'_k50_thres6_100eta'+str(int(100*args.eta))+args.outlier_style+'seed'+str(random_seed)+'split'+str(split)+'.png',format='png')
            plt.show()
            upset_full = [upset.detach().item(), cycle_inconsistency_val, ANE]
